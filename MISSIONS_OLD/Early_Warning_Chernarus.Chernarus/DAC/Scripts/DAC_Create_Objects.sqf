...
{		
	...
	{
		if(!isNil {_objArray select 0}) then
		{
			if((typeName (_objArray select 0)) != "ARRAY") then
			{
				if((count (triggerArea (_objArray select 0))) > 0) then
				{
					waituntil{!isNil {(_objArray select 0) getVariable "objCreated"}};
					sleep 0.1;
		...
		else
		{
			if(count _objArray > 1) then
			{
				if(count _this == 8) then
				{
					if((typeName (_objArray select 0)) == "ARRAY") then
					{
						if((typeOf ((_objArray select 0) select 0)) == "LOGIC") then
						{
					...
					{
						if((typeOf (_objArray select 0)) == "LOGIC") then
						{
			...
			{
				if(isNil {call compile _arrayName}) then {call compile format["%1 = []",_arrayName]} else {call compile _arrayName};
			};
			sleep 0.1;
			if(count _this == 8) then
			{
				...
				switch (_run) do
				{
					case 3:	{
								...
								while{_action} do
								{
									...
									sleep 0.1;
									if(({isNil "_x"} count [_wpConfig,_objConfig,_customWP]) == 0) then {_action = false};
								};
