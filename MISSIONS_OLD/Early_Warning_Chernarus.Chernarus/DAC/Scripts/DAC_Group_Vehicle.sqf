	...
	while {_run > 0} do
	{
		switch (_run) do
		{
			...
			case 4:	{
						...
						if(!isNil "_WeapList" && {_WeapList > 0}) then {[_leaderman,format["%1",_WeapList]] spawn DAC_Weapons;_WeapList = 0};
			...
			case 5:	{
						...
						if(!isNil "_WeapList" && {_WeapList > 0}) then {[_eman,format["%1",_WeapList]] spawn DAC_Weapons;_WeapList = 0};
						DAC_Init_Values set [1,(DAC_Init_Values select 1) + 1];
						if(((DAC_Com_Values select 1) == 2) && (!(DAC_InSpawn)) && ((getdir DAC_StartDummy) < 170)) then
						{
							DAC_InitText = (DAC_InitText + "|");
			...
			case 6:	{
						...
						if(DAC_Marker > 0  && {DAC_Marker_Val select 3 > 0}) then {[_leaderman,3,_group] spawn DAC_fGroupMarker};
