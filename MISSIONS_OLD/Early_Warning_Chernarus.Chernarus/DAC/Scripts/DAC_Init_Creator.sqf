...
_pa = 0;_pn = DAC_StartDummy; DAC_Players = call DAC_GetPlayers;
...
else
{
	While {_pa < count DAC_Players} do 
	{
		...
		if(!isNil {DAC_Players select _pa}) then
		{
...
if(count DAC_SaveDistance > 1) then
{
	if((typeName (DAC_SaveDistance select 1)) == "ARRAY") then
	{
		...
		{
			{if(isNil call compile "((DAC_SaveDistance select 1) select 0)") then {DAC_SaveDistance set[0,-1]}}foreach (DAC_SaveDistance select 1);
