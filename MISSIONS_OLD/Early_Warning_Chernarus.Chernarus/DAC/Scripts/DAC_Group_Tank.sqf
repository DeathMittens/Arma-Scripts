...
private [
			...
			"_unittyp","_crewcount","_ldriver","_lgunner","_lcommander","_vehiclepara","_groupvehicle","_mp",
		...
		switch (_run) do
		{
			...
			case 3:	{
						...
						else
						{
							...
							else
							{
								...
								if(!isNil "_WeapList" && {_WeapList > 0}) then {{[_x,format["%1",_WeapList]] execVM "DAC\Scripts\DAC_Change_Weapons.sqf"}foreach _crewA;_WeapList = 0};
			...
			case 6:	{
						...
						else
						{
							...
							if(DAC_Marker > 0  && {DAC_Marker_Val select 3 > 0}) then {[_ldriver,3,_group] spawn DAC_fGroupMarker};
						};
