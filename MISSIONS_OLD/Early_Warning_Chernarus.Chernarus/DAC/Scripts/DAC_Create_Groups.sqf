...
while {_run > 0} do
{
	switch (_run) do
	{
						...
						while {_check > 0} do
						{
							switch (_check) do
							{
								...
								case 5:	{
											...
											if((_DAC_CountMAX > 0) && {_max > (count _WPTempA + count _WPTempB)}) then {_max = count _WPTempA};
		...
		case 4:		{
						...
						if(_DAC_Group_Typ < 5) then
						{
							while{_action} do
							{
								...
								sleep 0.1;
								if(!isNil "_GroupPos") then {if(count _GroupPos > 0) then {_action = false}};
							};
		...
		case 8:		{
						waituntil {(!(DAC_InCreate)) && {DAC_Master_Init == 0} && {({(_x == 0)} count DAC_Init_Counter) == 4}};
						DAC_Init_Unit set [0,(DAC_Init_Unit select 0) - 1];
						DAC_Init_Group = DAC_Init_Group - [_DAC_Group_Typ];
						sleep 0.01;
						if(count DAC_Init_Group == 0) then {DAC_StartDummy setpos [100,100,0]};
