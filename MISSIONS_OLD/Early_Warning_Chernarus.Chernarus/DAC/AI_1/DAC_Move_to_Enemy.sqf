...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 4:	{
					...
					if((_groupid < 3) && {({alive _x} count units _group) <= _joinbehav} && {!(_group in DAC_Min_Groups)}) then
					{
					...
					while {_go > 6} do
					{
						...
						else
						{
							...
							else
							{
								...
								if((_go > 0) && {unitready vehicle _leader} && {(_leader distance _movepos) < 5}) then {_go = 0;_id = 3;_run = 7};
		...
		case 21:{
					...
					else
					{
						...
						if((_groupid != 6) && {count _this == 7} && {({alive _x} count units _group) == 0}) then
						{
