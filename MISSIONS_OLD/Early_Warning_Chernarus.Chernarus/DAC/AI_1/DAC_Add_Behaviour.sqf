...
if(!isNil {_group getVariable "getout_dac"}) then
{
		...
		case 4:	{
					...
					while {_go > 0} do
					{
						...
						else
						{
							...
							if((_go == 1) && {({alive _x} count units _group) == 0}) then {_go = 0;_run = 10};
							if((_go == 1) && {_group in DAC_Hit_Groups}) then {_go = 0;_run = 6};
							if(_go == 1) then
							{
