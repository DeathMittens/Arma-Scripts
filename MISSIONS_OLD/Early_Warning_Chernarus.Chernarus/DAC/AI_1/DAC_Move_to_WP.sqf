...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 4:	{
					...
					while {_go > 0} do
					{
						...
						else
						{
							...
							if((_go == 1) && {unitready (vehicle _leader)} && {((vehicle _leader) distance _nextpos) < 20}) then {_go = 0;_run = 5};
		...
		case 5:	{
					...
					if(_groupid == 1) then 
					{
						[_leader,1] spawn DAC_fAISpeak;[0,(leader _group)] spawn DAC_fAIRadio;if((behaviour _leader) != "CARELESS") then {{_x setunitpos "middle"} foreach units _group};
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								if((_groupid == 1)&& {({alive _x} count units _group) <= _joinbehav} && {!(_group in DAC_Min_Groups)}) then {_run = 7};
								if((_groupid == 1) && (_run == 6) && {(_bldgbehav select 0) > 0} && {({_x == vehicle _x} count units _group) > 1}) then {_run = 9};
								if((_groupid == 1) && {_run == 6} && {(_emptyVeh > 0) || (_emptyGun > 0)} && {({_x == vehicle _x} count units _group) > 1} && {({alive _x} count units _group) > 1}) then {_run = 11};
		...
		case 14:{
					...
					while {_go > 0} do
					{
						...
						if(_go == 1) then
						{
							...
							if((isNull _newtarget) || {(typeof _newtarget) == ""}) then
							{
								_id = 0;
							}
							else
		...
		case 21:{
					...
					if(!isNil "_jArray" && {(count _jArray > 0)}) then {[35,_groupid,_group,"P",(leader (_jArray select 0)) distance player] spawn DAC_fShowRadio};
					_run = 0;		
				};
		Default{};
	};
