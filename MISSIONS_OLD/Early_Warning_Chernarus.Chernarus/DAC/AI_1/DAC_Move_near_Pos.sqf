...
private [
			...
			"_marker","_DAC_Arrays","_callg","_callE","_ce","_kw","_ct","_run","_go","_val","_newtarget","_sum","_id","_defined"
		];
			...
			_run = 1;_go = 0;_DAC_Arrays = ["DAC_Help_Groups","DAC_Hit_Groups","DAC_Fire_Groups","DAC_Radio_Groups","DAC_Cover_Groups","DAC_Call_Groups"];_sum = 0; _defined = false;
	...
	switch (_run) do
	{
		...
		case 4:	{
					...
					while {_go > 0} do
					{
						...
						else
						{
							...
							if((_go == 1) && {unitready (vehicle _leader)} && {((vehicle _leader) distance _nextpos) < 5}) then {_go = 0;_run = 5};
		...
		case 6:	{
					...
					else
					{
						...
						{
							if _defined && {(_checkpos2 - _checkpos1) < 2} && {(_checkpos2 - _checkpos1) > -2} && {(speed vehicle _leader) < 3)} then {
							{
								_run = 8;
							}
							else
