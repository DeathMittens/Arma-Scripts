...
private [
			...
			"_target","_usercondition","_tempV","_tempA","_n","_f","_d","_cArray","_val","_sArray","_uArray","_tArray","_maxDistance"
		];

			_callGroup = _this select 0;_enemyGroup = _this select 1;_i = 0;_veh = [];_random = 0;_a = 0;_v = 0;_condition = [];
			_artiArray = _this select 2;_parameter = [];_artiUnit = DAC_StartDummy;_addcondition = [];
			_leader = leader _callGroup;_s = 0;_h = 0;_c = 0;_artiVeh = [];_nObj = [];_nPos = position DAC_StartDummy;_maxI = 0;
			_tArray = [];_tempV = [];_tempA = [];_n = 0;_f = 0;_target = _this select 3;_d = _this select 4;_maxDistance = 0;_maxA = 0;
			_cArray = [];_sArray = [];_val = 0;_uArray = [];_countbullets = 0;

if((((_artiArray select 2) select 3) == 0) || {_enemyGroup in DAC_Arti_Array}) then
{
	if(time > 0) then
	{
...
{
	_uArray set [count _uArray,str(side _leader)];
	switch (str (side _leader)) do
	{
	...
	while {_i < 3} do
	{
		switch (_i) do
		{
			...
			case 1:	{
						...
						if((_i < 3) && ((count (_condition select 3)) > 0)) then {if(({(alive _x) && {(typeof _x) in (_condition select 3)}} count (units _callGroup)) == 0) then {_i = 4;_h = 6}};
			...
			case 2:	{
						...
						if((_i < 3) && {((count _addcondition) > 0) || ((count _artiVeh) > 0)}) then
						{
							...
							while {_v < count _list} do
							{
								...
								if(!isNull (gunner _artiUnit)) then
								{
									if((str (side (gunner _artiUnit))) in _uArray) then
									{
										if((alive _artiUnit)
										&& {someammo _artiUnit}
										&& {(typeof _artiUnit) in _tempV}
											...
											if((_artiUnit distance _target) < _maxDistance) then
											{
												if(!(_artiUnit in DAC_Arti_Veh)) then
												{
													if(!(_artiUnit in _tArray)) then
													{
														if((_condition select 6) >= 1) then
														{
															_f = _tempV find (typeof _artiUnit);
															if(_f >= 0) then
															{
														...
														{
															if(isNull (driver _artiUnit)) then
															{
																_f = _tempV find (typeof _artiUnit);
																if(_f >= 0) then
																{
	...
	if(_a == 3) then
	{
		if(((typeof _target) == "LOGIC") || {({alive _x} count units _enemyGroup) > 0}) then
		{	
