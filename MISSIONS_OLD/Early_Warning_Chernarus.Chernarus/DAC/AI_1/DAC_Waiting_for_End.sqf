...
private [
			...
			_tempcamp = objNull; _temptyp = objNull; _tempcount = objNull; _tempother = objNull; _tempvalue = objNull;
			_tempvalue2 = objNull; _building = objNull; _camp = objNull; _shet = objNull;
			
if(!isNull _group) then
{			
	if(count _this == 8) then {_camparray = _this select 6;_marker = _this select 7;_delPos = position (_camparray select 0)};
	if(isNull (driver vehicle (leader _group))) then {_groupid = 8};
	sleep 1;
	{_x addeventhandler ["hit",{_this spawn DAC_fHitByEnemy}]} foreach units _group;
	if((_group getVariable "getout_dac") == 1) then {_group setVariable ["getout_dac", 0, false]};
		...
		switch (_run) do
		{
			...
			case 2:	{
						...
						while {_go} do
						{
							...
							{
								if((!alive _x) && {!(_x in _delarray)}) then
								{
							...
							if((_group getVariable "getout_dac") == 1) then {_run = 11;_go = false};
							sleep (1 + (random 1));
						};
					};
			case 3:	{
			...
			case 4:	{
						...
						else
						{
							...
							_tO = nearestObjects [_delPos, [], 100];
							{
								...
								&& {(typeof _x) != "Logic"}
								&& {(typeof _x) != "Camera"}
								&& {(typeof _x) != "EmptyDetector"}) then {deletevehicle _x};
							}	foreach _tO;
			...
			case 6:	{
						...
						while {_go} do
						{
							...
							else
							{
								...
								if(!isNull _tempbuild) then
								{
									if((_tempcount > 0) && {format["%1",call compile _tempvalue] == format["%1",call compile _tempvalue2]}) then
									{
										if(_temptyp == 0) then
										{
			...
			case 9:	{
						...
						while {_go} do
						{
							...
							{
								if(({(alive _x) && {(_x distance _building) < (DAC_AI_Spawn select 4)}} count DAC_Players) > 0) then
								{
									(((call compile _dc) select _rdmcamp) select 5) set [0,(((((call compile _dc) select _rdmcamp) select 5) select 0) + 1)];
									(((call compile _dc) select _rdmcamp) select 5) set [1,(((((call compile _dc) select _rdmcamp) select 5) select 1) - 1)];
									sleep (random 1);
								...
								{
									sleep (1 + random 1);
									if((!(DAC_InSpawn)) && {!(_building in DAC_Camps_InUse)}) then {_run = 10;_go = false};
								};
							};
						};
					};
