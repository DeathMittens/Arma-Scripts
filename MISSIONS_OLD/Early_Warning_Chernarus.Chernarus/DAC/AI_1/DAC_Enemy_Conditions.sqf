...
if(count _this == 3) then
{
	if(!(isNull(_leader findNearestEnemy (position _leader)))) then
	{
		...
		if((alive _newtarget) && {_newtarget != _target} && {((behaviour _newtarget) != "ERROR")}) then
		{
			if(({(alive _x) && {(_x knowsabout _newtarget) > _knowsabout} && {(_x distance _newtarget) < (call compile DAC_Alert_Dist)}} count units _group) > 0) then
			{
	...
	if(!(isNull(_leader findNearestEnemy (position _leader)))) then
	{
		...
		if((alive _newtarget) && {_newtarget != _target} && {(behaviour _newtarget) != "ERROR"}) then
		{
			if((count crew _newtarget) > 0) then
			{
				...
				if((_t1 > _t2) && (({(alive _x) && {(_x distance _newtarget) < (call compile DAC_Alert_Dist)}} count units _group) > 0)) then
				{
					_nexttarget = _newtarget;
				}
				else
