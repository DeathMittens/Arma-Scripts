...
_side = ["EAST","WEST","GUER","CIV"];_cv = 0;_cw = 1;_z = 0;_aMC = [];_fs = "";
...
{
	_fs = _side find str(side (leader _group));
	...
	{			
		...
		if(_c > 0) then
		{
			...
			while {_count > 0} do
			{
				_conditions = [_group,_entry,_artiArray,_target,_d] call DAC_fArtiConditions;
				sleep 0.1;
				...
				else
				{
					...
					if(DAC_Marker > 0) then
					{
						...
						{
							_articolor = DAC_Marker_Val select 14;
