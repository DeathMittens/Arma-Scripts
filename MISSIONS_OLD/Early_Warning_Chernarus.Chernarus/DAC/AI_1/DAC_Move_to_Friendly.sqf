...
if(!isNil {_grouponfire getVariable "support_pos"}) then {_firemark = _grouponfire getVariable "support_pos"};
	...
	switch (_run) do
	{
		...
		case 9:	{
					...
					while {_go > 0} do
					{
						...
						if((_go == 1) && {({alive _x} count units _group) == 0}) then {_go = 0;_run = 14};
						if((_go == 1) && {_group in DAC_Hit_Groups}) then {_go = 0;_run = 12};
						if(_go == 1) then
						{
		...
		case 15:{
					...
					{
						if(((_groupid < 3) || (_groupid == 5)) && {({alive _x} count units _group) > 0}) then {[_leader,5] spawn DAC_fAISpeak;[11,(leader _group)] spawn DAC_fAIRadio;};
