...
if((DAC_Marker_Val select 10) <= 0) exitwith {};
...
_CampList = format["%1",_SideCP select _SideID];_p = [0,0,0];_rnd = 0;
		...
		else
		{
			...
			if(count _temp > 0) then
			{
				...
				while {_cc4 < count _temp} do
				{
					...
					//if((DAC_Marker_Val select 0) == 2) then {[[_la],2,_lr,_lt,_flag] spawn DAC_fFlashMarker};
