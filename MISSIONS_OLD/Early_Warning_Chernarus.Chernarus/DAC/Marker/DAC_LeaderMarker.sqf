...
_unit = _this select 0;_group = _this select 1;_leader = leader _group;_check = true;_run = 0;_go = true;_wppos  = position _leader;

waituntil {(getdir DAC_StartDummy) > 170};

if(_unit == _leader) exitwith {};
if (DAC_Marker > 0 && {(DAC_Marker_Val select 3) == 0}) then {_run = 1};
	...
	switch (_run) do
	{
		case 0:	{
					while {_run == 0} do
					{
						...
						{
							if ((DAC_Marker_Val select 8) > 0) then {_run = 1;_go = true};
						};
					};
				};
		case 1:	{
					...
					while {_go} do
					{
						...
						else
						{
							...
							else
							{
								...
								{
									if((_unit == leader _group) || (!(_group == group _unit)) || ((count (waypoints (leader _group))) < 2)) then
									{
										deletemarkerlocal _ma;_check = false;_go = false;
									}
									else
